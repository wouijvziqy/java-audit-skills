...
3. **⚠️ 完整性校验（必须执行）**：
   ...
   - **若一致**：继续执行步骤 4
4. **路由数量判断**：基于「筛选概览」中的**高危路由总数**（而非待追踪列表行数），按以下规则处理：

   **情况 A：路由总数 ≤ 1500 条** — 直接进入步骤 5 分批，无需筛选
...
5. 按以下规则分批（对全量或精选后的路由）：
   - 每批最多 10 条路由
   - 按 P0→P1 优先级排序后顺序分批（不拆散同一优先级）
   - 若总路由数 ≤ 10，仅生成 1 个批次
6. 为每个批次生成完整的 worker 指令（含路由列表 + 鉴权风险信息）
7. 输出 `trace_batch_plan.md` 后通知负责人

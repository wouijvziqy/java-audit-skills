...
⚠️ **完整性强制要求**：此列表必须包含上方 P0 和 P1 分组表格中的**全部路由**，禁止做任何筛选、去重或缩减。待追踪路由总数必须 = P0 数量 + P1 数量 = 筛选概览中的「高危路由总数」。
...
- **agent-4a 的职责仅为分级，不做筛选**：所有 P0+P1 路由必须全量输出到「待追踪路由列表」，数量裁剪决策由 agent-5 负责

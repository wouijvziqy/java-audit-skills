...
- 检查：鉴权框架识别、组件版本检查、路由鉴权映射表（✅/⚠️/❌）、风险统计、高危详情+PoC
...
- 检查：扫描概览、模块风险摘要、漏洞详情（CVE）、触发点检测章节
...
- 追踪覆盖率：已追踪数 / high_risk 待追踪数（P0+P1） >= 90%
...
- ✅ agent-5-1: java-route-tracer 校验通过（含鉴权状态透传）[agent-7-1]
- ✅ agent-5-2: java-route-tracer 校验通过（含鉴权状态透传）[agent-7-2]
- ✅ ...（逐个 worker 校验记录）
- ✅ 追踪覆盖率: 95% >= 90%

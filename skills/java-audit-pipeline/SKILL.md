...
2. 创建输出目录结构（一次性创建所有子目录）：
   ```bash
   mkdir -p {output_path}/route_mapper {output_path}/auth_audit {output_path}/vuln_report {output_path}/cross_analysis {output_path}/route_tracer {output_path}/sql_audit {output_path}/xxe_audit {output_path}/file_upload_audit {output_path}/file_read_audit {output_path}/decompiled {output_path}/scripts
   ```
3. 创建 agent team
...
负责人创建 agent-4a 时，读取 `references/agent_4a_instructions.md` 获取完整执行步骤和输出模板，将其作为 agent-4a 的 prompt 指令。

---

### Agent-4b-vuln-aggregator: 漏洞汇总员

负责人创建 agent-4b 时，读取 `references/agent_4b_instructions.md` 获取完整执行步骤和输出模板，将其作为 agent-4b 的 prompt 指令。

---

### Agent-5-route-tracer: 调用链追踪分配员

负责人创建 agent-5 时，读取 `references/agent_5_instructions.md` 获取完整执行步骤、智能精选策略和输出模板，将其作为 agent-5 的 prompt 指令。
...
4. 这些信息将被 agent-6 系列读取，用于判定漏洞的可利用性
...
6. 不合格 → 将完整校验报告发送给对应 agent，要求按不通过项清单补充
7. 合格 → 将完整校验报告发送给负责人，确认该 agent 校验通过
